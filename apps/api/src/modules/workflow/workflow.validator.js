...
export function validateCreateWorkflowStep(payload) {
  if (!Number.isInteger(Number(payload.order)) || Number(payload.order) < 1) throw new Error("Step order must be a positive integer");
  if (!payload.name?.trim()) throw new Error("Step name is required");
  if (!ACTIONS.includes(payload.action)) throw new Error(`Invalid workflow action: ${payload.action}`);
  if (!payload.position) throw new Error("Workflow position is required");
}
export { ACTIONS };
