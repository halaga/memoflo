...
class WorkflowService {
  ...
  async addStep(
    ...
    const order = requestedOrder > 0 ? requestedOrder : steps.length + 1;
    const occupied = steps.some((step) => Number(step.order) === order);
    if (occupied) {
      await WorkflowRepository.shiftStepsFromOrder(workflowId, order);
    }
    return WorkflowRepository.createStep({ ...payload, order, workflow: workflowId });
  }
